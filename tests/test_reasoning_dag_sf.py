"""Three-layer reasoning DAG integration tests — Case 3 (SF + resistor load).

Verifies that SF_RESISTOR_TEMPLATE + execute_reasoning_dag + rules.py
produce values matching golden_sf_resistor_load.yaml within 5% tolerance
...
from src.solver.templates import SF_RESISTOR_TEMPLATE
from tests.golden_helpers import load_golden

GOLDEN_PATH = Path("tests/golden/golden_sf_resistor_load.yaml")
TOLERANCE = 0.05
...
def dag_trace(golden) -> ReasoningTrace:
    ...
    given = {
        ...
        "Cgd":      float(g["Cgd"]),
        "Cgs":      float(g["Cgs"]),
        "Rs_source":float(g["Rs_source"]),
        "gmb":      float(g["gmb"]),
    }
    return execute_reasoning_dag(SF_RESISTOR_TEMPLATE, given)
...
def test_qpoint_VS(dag_trace, golden):
    _assert_close(
        dag_trace.final_values["VS"],
        float(golden["expected"]["qpoint"]["VS"]),
        "VS",
    )
...
def test_small_signal_one_over_gm(dag_trace, golden):
    _assert_close(
        dag_trace.final_values["one_over_gm"],
        float(golden["expected"]["small_signal"]["one_over_gm"]),
        "one_over_gm",
    )
...
def test_low_freq_Req(dag_trace, golden):
    _assert_close(
        dag_trace.final_values["Req"],
        float(golden["expected"]["low_frequency"]["Req_for_av"]),
        "Req",
    )
...
def test_final_values_contains_all_given_keys(dag_trace):
    expected_keys = {
        "VDD", "Rs_load", "VG_DC", "Vth", "kn", "lambda", "CL",
        "Cgd", "Cgs", "Rs_source", "gmb",
    }
...
def test_trace_length_matches_template(dag_trace):
    assert len(dag_trace.entries) == len(SF_RESISTOR_TEMPLATE)
